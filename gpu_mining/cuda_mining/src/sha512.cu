...
__global__ void sha512(const uint_64* __restrict__ d_input, uint_64* d_output, unsigned int length) {
...
extern "C" const uint_64  SHA512(const uint_64 prev_proof, const char *proof_of_work) {
    ...
    uint_64* h_result = new uint_64[8 * hash_elements];

    uint_64 *d_input;
    cudaMalloc(&d_input, sizeof(uint_64) * 16 * hash_elements);

    uint_64 *d_output;
    cudaMalloc(&d_output, sizeof(uint_64) * 8 *hash_elements);
